...
class Layer():
    ...
    def check_reuse(self,scope):
        if self.call_cnt >0:
            ...
            if self.reuse == None:
                if self.call_cnt >0:
                    print ("Warning: Reuse variable with reuse value = None in scope",scope.name,scope.reuse_variables())
            if self.reuse == False:
                if self.call_cnt >0:
                    print ("Error: reuse variable with reuse value = False in scope",scope.name,exit(-1))
        self.call_cnt +=1
