# -*- coding: utf-8 -*-
"""
Created on Thu Nov 15 09:49:29 2018

@author: zhenjj2
"""

#BERT中文文本分类
#https://daiwk.github.io/posts/nlp-bert-code.html
#https://mp.weixin.qq.com/s/5gxPFNxZUXAcZEGiZj1LpA
#参考论文：https://arxiv.org/pdf/1805.00250.pdf
#参考github网址：https://github.com/sanmusunrise/AdaScaling
#主要改动地方在loss和添加class MaskedSoftmaxLayer(Layer)类中，参考调代码的尝试文件为：D:\G-工作盘\Telesales项目\许洋项目数据\code\AdaScaling-master\CNN\tfnlp\layer\MaskedSoftmaxLayer.py
...
class telesalesProcessor(DataProcessor):
  def __init__(self):
    self.language = "zh"
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    lines = self._read_tsv(
        os.path.join(data_dir,"trainFolder1.tsv" )) 
    examples = []
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    lines = self._read_tsv(os.path.join(data_dir,"devFolder1.tsv"))
    examples = []
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
...
class resProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    '''
    细粒度用户评论情感分析的数据处理
    #首先生成每行一个ID的数据，取消每个ID中存在'\n'的内容
    input_file='C:/Users/zhenjj2/Desktop/sentiment_analysis_testb.csv'
    train=pd.read_csv(input_file)
    train.replace('\n','',regex=True,inplace=True)
    train.to_csv('sentiment_analysis_testb11.csv',index=False)
    '''
    lines = self._read_csv(
        os.path.join(data_dir, "data/",
                     "sentiment_analysis_trainingset11.csv" ),quotechar='"') 
    examples = []
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    lines = self._read_csv(os.path.join(data_dir, "data/","sentiment_analysis_validationset11.csv"),quotechar='"')
    examples = []
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
  ...
  def get_test_examples(self, data_dir):
    """See base class."""
    lines=self._read_csv(os.path.join(data_dir,'data/',"sentiment_analysis_testa11.csv"),quotechar='"')
    examples = []
    for (i,line) in enumerate(lines):
        if i==0:
            continue
...
class ColaProcessor(DataProcessor):
  ...
  def _create_examples(self, lines, set_type):
    ...
    examples = []
    for (i, line) in enumerate(lines):
      # Only the test set has a header,test的header为[index	sentence]
...
def convert_single_example(ex_index, example, label_list, max_seq_length,tokenizer):
  ...
  if tokens_b:
    ...
    # length is less than the specified length.将tockens_a tockens_b处理成max_seq_length - 3大小的，如果tockens_a和tockens_b都超出，则均分，如果tockens_a中10个字符，tockens_b中20个字符，max_seq_length=9，则二者共同为6个字符，均分到tockens_a和tockens_b上分别为前3个，（9-3）/2=3个
...
def file_based_convert_examples_to_features(
  ...
  先是对字符串长度，加入[CLS]和[SEP]等一些处理后，将其写入成TFrecord的形式。
  这是为了能在estimator里有一个更为高效和简易的读入。"""
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    feature = convert_single_example(ex_index, example, label_list,
                                     max_seq_length, tokenizer)  #Converts a single `InputExample` into a single `InputFeatures
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    p1 = tf.reduce_sum(positive_idx * golden_prob) #TP,golden_prob其实可以等价于logits，即softmax_layer到golden_prob的代码忽略，直接将golden_prob使用logits代替
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  """Returns `model_fn` closure for TPUEstimator.后续在GPU上运行时候需要将参数tf.contrib.tpu.TPUEstimator更改为tf.estimator.Estimator"""
...
def main(_):
  ...
  tokenizer = tokenization.FullTokenizer(
      vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)  #21128个字 ('傭', 999),
...
#data_dir=args.data_dir, path_to_data=args.path_to_mrpc
